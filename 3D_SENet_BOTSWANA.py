...
# 加载数据
mat_data = sio.loadmat('F:/transfer code/Tensorflow  Learning/3D-SENet/datasets/Botswana/Botswana.mat')
data_IN = mat_data['Botswana']
# 标签数据
mat_gt = sio.loadmat('F:/transfer code/Tensorflow  Learning/3D-SENet/datasets/Botswana/Botswana_gt.mat')
...
img_channels = 145
PATCH_LENGTH = 9  # Patch_size (13*2+result)*(13*2+result)
...
OA_3D_SENet = []
AA_3D_SENet = []
TRAINING_TIME_3D_SENet = []
TESTING_TIME_3D_SENet = []
ELEMENT_ACC_3D_SENet = np.zeros((ITER, CATEGORY))
...
for index_iter in range(ITER):
    ...
    best_weights_SENet_path = 'F:/transfer code/Tensorflow  Learning/3D-SENet/models-botswana-19-3-217/Indian_best_3D_SENet_' + str(
        index_iter + 1) + '.hdf5'
    ...
    saveBestModel6 = kcallbacks.ModelCheckpoint(best_weights_SENet_path, monitor='val_loss', verbose=1,
                                                save_best_only=True,
                                                mode='auto')
    ...
    history_3d_SENet = model_SENet.fit(
    ...
    # 测试
    tic7 = time.clock()
    loss_and_metrics = model_SENet.evaluate(
        x_test.reshape(x_test.shape[0], x_test.shape[1], x_test.shape[2], x_test.shape[3], 1), y_test,
        batch_size=batch_size)
    toc7 = time.clock()

    print('3D SENet Time: ', toc6 - tic6)
    print('3D SENet Test time:', toc7 - tic7)

    print('3D SENet Test score:', loss_and_metrics[0])
    print('3D SENet Test accuracy:', loss_and_metrics[1])

    print(history_3d_SENet.history.keys())

    # 预测
    pred_test = model_SENet.predict(
    ...
    KAPPA_3D_SENet.append(kappa)
    OA_3D_SENet.append(overall_acc)
    AA_3D_SENet.append(average_acc)
    TRAINING_TIME_3D_SENet.append(toc6 - tic6)
    TESTING_TIME_3D_SENet.append(toc7 - tic7)
    ELEMENT_ACC_3D_SENet[index_iter, :] = each_acc

    # 绘制acc-loss曲线
    history.loss_plot('epoch')

    print("3D SENet finished.")
    print("# %d Iteration" % (index_iter + 1))

# 自定义输出类
modelStatsRecord.outputStats(KAPPA_3D_SENet, OA_3D_SENet, AA_3D_SENet, ELEMENT_ACC_3D_SENet,
                             TRAINING_TIME_3D_SENet, TESTING_TIME_3D_SENet,
                             history_3d_SENet, loss_and_metrics, CATEGORY,
                             'F:/transfer code/Tensorflow  Learning/3D-SENet/records-botswana-19-3-217/IN_train_3D.txt',
                             'F:/transfer code/Tensorflow  Learning/3D-SENet/records-botswana-19-3-217/IN_train_3D_element.txt')
